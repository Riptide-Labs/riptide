...
| **`riptide.discovery.poll-always-tag`** | string | unset | Read from the `__meta_netbox_tags` label. The `netbox-api` source emits that label from a device's tag slugs; a `prometheus-sd` document that carries the same label is honoured identically; `mapped-json` never carries it. A claiming entry is composed with `poll: always`, so it is polled for SNMP metrics without waiting for a flow. See [Exporter enrichment](exporter-enrichment.md). |
