...
Riptide fills `srcCountry`, `srcCity`, `dstCountry` and `dstCity` on every flow, and AS number and organisation where nothing above GeoIP on the [enrichment ladder](../architecture/enrichment.md#the-enrichment-ladder) supplied them.
