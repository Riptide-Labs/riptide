...
| **`riptide.snmp.poll.suspect-pool-width`** | int | `8` | Walks in flight for agents whose last walk failed. Kept apart from `pool-width`, so dead agents waiting out their timeouts cannot hold back healthy ones. A due walk that finds its budget busy waits in a due queue, in due order, and starts the moment a permit frees; each second it waits is counted on `snmp.poller.deferred`. |
