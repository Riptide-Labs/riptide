...
| **`riptide.snmp.poll.pool-width`** | int | `4` | Walks in flight across the whole fleet for agents whose last walk succeeded. Per-agent concurrency is always one. A walk takes no thread while it waits on the agent, with one exception. An SNMPv3 walk that does not yet know the agent's engine ID runs a discovery first, which blocks one `snmp-walk-io` thread for up to one second. With `collect` set this happens on an agent's first walk and again after any failed walk. Every enrichment-only walk repeats it. Sizing is on the [SNMP metrics reference](snmp-metrics.md#size-the-permits). |
| **`riptide.snmp.poll.suspect-pool-width`** | int | `8` | Walks in flight for agents whose last walk failed. Kept apart from `pool-width`, so dead agents waiting out their timeouts cannot hold back healthy ones. A due walk that finds its budget busy waits in a due queue, in due order, and starts the moment a permit frees; a walk still waiting when the next tick comes is counted on `snmp.poller.deferred`. |
...
| **`riptide.snmp.poll.max-exporters`** | int | `4096` | Registered agents, flow-registered and `poll: always` alike. At the cap a new flow-registered exporter is rejected and counted on `snmp.poller.rejectedLookups`; nothing is evicted. If the inventory's `poll: always` entries alone exceed this cap, none of them is registered, and the refusal is logged and counted on `snmp.poller.inventoryRefused`. If they fit but flow-registered exporters already fill the cap, each entry that finds no room is not registered, and those entries are counted on the same gauge with one warning per inventory sweep. |
