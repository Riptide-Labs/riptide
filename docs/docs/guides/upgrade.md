...
- A deployment in validate mode (`riptide.clickhouse.manage-schema=false`) has the admin credential for `riptide onboard` at hand; see [Onboard a tenant](onboard-a-tenant.md).
...
1. Replace the running version.
   ...
   | deb and rpm | `apt upgrade riptide` or `dnf upgrade riptide`; see [Linux packages](linux-packages.md) |
   | NixOS | bump the flake input; see [NixOS](nixos.md) |
...
See [Query sampling-corrected volume](sampling-corrected-volume.md) for the predicates a rollup query needs.
...
Migrate one tenant at a time and close each database with `revoke-legacy`, in the order [Migrate a deployment onboarded before the rename](migrate-tenant-accounts.md) gives; dropping the old account too early takes every other database of that tenant offline.
