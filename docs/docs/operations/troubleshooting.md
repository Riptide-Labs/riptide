...
| Startup fails: `<file> declares an 'exporters' tree while riptide.discovery.url is set` | Both the inventory file and discovery define exporters | Remove the `exporters` tree from the file, or unset `riptide.discovery.url`. See [What discovery owns](../architecture/discovery.md#what-discovery-owns). |
