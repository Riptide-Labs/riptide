...
With [dynamic discovery](discovery.md) enabled, `agents: {}` and `riptide: {}` still mean what they say, but `exporters: {}` does not apply: the file may not declare an exporters tree at all while discovery owns it.
