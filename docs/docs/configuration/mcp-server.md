...
- **1-Minute Rollup Query Router**: Queries spanning $\ge 60$ minutes are automatically routed to ClickHouse `SummingMergeTree` rollups (`flows_by_application_1m`, `flows_by_conversation_1m`, `flows_by_exporter_iface_1m`, `flows_by_geo_asn_1m`).
  The rollups carry `samplingInterval`, so a sampling-corrected query is the same expression whichever table a request lands on — except on deployments receiving sFlow, where the rollup form is unavailable because no rollup carries `flowProtocol` to exclude those rows with. The tools themselves report counters as the exporter reported them and do not apply the correction — see [sampling-corrected volume](receivers#sampling-corrected-volume-beyond-raw-retention).
