...
A database onboarded before the rename still has the unqualified `bi_<tenant>` account until you re-run `onboard`, see [Object names carry their database](../deploy/multi-tenancy.md#object-names-carry-their-database).
...
The tools report counters as the exporter sent them and do not apply that correction, see [sampling-corrected volume](receivers.md#sampling-corrected-volume-beyond-raw-retention).
