...
Fields left unset fall through, so pinning only `alias` keeps the `name` and `high-speed` that option data or the SNMP walk supply; see the [enrichment ladder](../enrichment.md#the-enrichment-ladder).
