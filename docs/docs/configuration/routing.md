...
It is the [enrichment ladder's](../enrichment.md#the-enrichment-ladder) rung for AS data between the exporter's own BGP view and [GeoIP](geoip.md).
...
With **`riptide.config.reload-interval`** set, the config file is hashed on every poll and both tables are re-bound whenever its content changes; see [config hot-reload](../deploy/operations.md#config-hot-reload).
...
A commit whose inventory rebuild is still pending counts on `config_reload_partial` and keeps the gauge at `1.0`; the [reload metrics](../deploy/operations.md#config-hot-reload) table has the full contract.
