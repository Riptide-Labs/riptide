...
| --- | --- |
| Java runtime | 25 |
| ClickHouse | reachable at `riptide.clickhouse.endpoint`, default `http://localhost:8123`; see [startup wait](../configuration/clickhouse.md#startup-wait) for what happens while it is not |
...
| --- | --- | --- |
| **`/etc/riptide/config.yaml`** | YAML, imported by the bundled `spring.config.import=optional:file:/etc/riptide/config.yaml` | yes, with [config hot-reload](operations.md#config-hot-reload) |
...
Agent ranges and, with [dynamic discovery](../configuration/discovery.md) off, enrichment entries exist only in the [inventory file](../configuration/agent-configuration.md).
...
Secret references work the same from every source, see [secret references](../configuration/secret-references.md).
...
| `java -jar riptide-flows-%%VERSION%%.jar onboard ...`, `offboard ...`, `revoke-legacy ...` | Provision a tenant on ClickHouse | [Multi-tenancy](multi-tenancy.md) |
