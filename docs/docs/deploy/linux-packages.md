...
3. Edit **`/etc/riptide/config.yaml`**. Point it at ClickHouse over the HTTP interface and define at least one receiver. Nothing starts until you do.
   ...
   The password is a [secret reference](../configuration/secret-references.md), so no plaintext lives in the file.
   Leave it unset for the `default` user's empty password.
   A ClickHouse credential that cannot be resolved fails startup.
   With `manage-schema: true`, the default, riptide creates the database and the `flows` table on first start; the user needs `CREATE`. See [ClickHouse](../configuration/clickhouse.md).
...
A change in `config.yaml` is picked up by [config hot-reload](operations.md#config-hot-reload) when it is enabled.
