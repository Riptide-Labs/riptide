 ...
 *
 * <p>Reachability differs per matcher and the rows say so. Only the protocol one is measured live with
 * the ruleset riptide ships: {@code ClassificationEnricher} builds every request through
 ...
 * {@code ClassificationEnricherTest}. The port and address siblings are latent <em>against that
 * ruleset</em>, on two measurements: a probe with no ports at all classified without reaching a port
 * matcher, and the bundled ruleset names no addresses. Neither is a structural guarantee — a leaf can
 * sit on a path that carries no threshold for the aspect it matches — and a ruleset is operator-supplied
 * data, so all three are reachable through the real engine, which is what the {@code aRuleNaming...}
 * rows in {@code DefaultClassificationEngineTest} show.
 *
...
class AbsentAspectIsNoMatchTest {
     ...
     * {@code IpRange.contains}, which calls {@code begin.compareTo(addr)}; {@code Ip4Addr.compareTo}
     * casts its argument and reads that argument's primitive field, so the throw was a plain null
     * dereference of the address passed in rather than an unboxing.
     */
    @Test
