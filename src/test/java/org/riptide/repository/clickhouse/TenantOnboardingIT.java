...
public class TenantOnboardingIT {
    ...
    void revokeLegacyClosesAMigratedDatabaseAndLeavesAnUnmigratedOneAlone() throws Exception {
        try (var admin = adminClient()) {
            ...
            // carrying the grants the pre-rename recipe left on it. Both roles, so the two-holder
            // report renders and the reader's revoke path is exercised — with the writer alone,
            // flow_reader's half of the statements is never generated by any test.
            Assertions.assertThat(ProvisioningCommand.run(onboardArgsFor("mig", "mgt", true),
                    discard(), discard())).isZero();
            admin.execute("CREATE ROLE IF NOT EXISTS flow_reader").get();
            admin.execute("CREATE USER bi_unmig IDENTIFIED WITH sha256_password BY 'legacyR'").get();
            admin.execute("GRANT flow_reader TO bi_unmig").get();
            for (final String table : ProvisioningDdl.legacyGrantTables("mig")) {
                ...
                admin.execute("GRANT SELECT ON " + table + " TO flow_reader").get();
            }
            ...
            try (var legacy = rawClientOn("mig", "bi_unmig", "legacyR")) {
                Assertions.assertThat(legacy.queryAll("SELECT count() AS c FROM "
                        + FlowsSchema.qualifiedFlows("mig")).getFirst().getLong("c")).isPositive();
            }

            // --dry-run first: it must print the statements, and change nothing at all.
            final var dry = new ByteArrayOutputStream();
            Assertions.assertThat(ProvisioningCommand.run(
                    revokeLegacyDryRunArgs("mig"), discard(),
                    new PrintStream(dry, true, StandardCharsets.UTF_8))).isZero();
            Assertions.assertThat(dry.toString(StandardCharsets.UTF_8))
                    .contains("Would revoke")
                    .contains("REVOKE INSERT, SELECT, SHOW TABLES ON " + FlowsSchema.qualifiedFlows("mig")
                            + " FROM `flow_writer`");
            try (var legacy = rawClientOn("mig", "writer_unmig", "legacyW")) {
                // No assertThatCode wrapper: an exception here fails the test, and the point is that
                // --dry-run left this working.
                legacy.execute("INSERT INTO " + FlowsSchema.qualifiedFlows("mig")
                        + " (tenant, organisation, timestamp) VALUES ('unmig', 'unmig-eu', now())").get();
            }
            ...
            Assertions.assertThat(err.toString(StandardCharsets.UTF_8))
                    ...
                    .as("both holders are reported, not just the writer")
                    .contains("flow_reader")
                    .contains(FlowsSchema.qualifiedFlows("mig"))
                    .contains(FlowsSchema.qualifiedRollup("mig", FlowsSchema.rollupTableNames().getFirst()))
                    .as("the rollup views are named too, not only the targets")
                    .contains(FlowsSchema.qualifiedRollupView("mig", FlowsSchema.rollupTableNames().getFirst()));
            ...
            // The legacy READER loses it too. Its own path, because the reader's revoke statements
            // are generated only when flow_reader is among the holders.
            try (var legacy = rawClientOn("mig", "bi_unmig", "legacyR")) {
                Assertions.assertThatThrownBy(() -> legacy.queryAll("SELECT count() AS c FROM "
                                + FlowsSchema.qualifiedFlows("mig")))
                        .as("the pre-rename reader must lose the migrated database as well")
                        .hasStackTraceContaining("ACCESS_DENIED");
            }
            ...
            // And the migrated database still works FOR ITS OWN TENANT. The other failure direction,
            // and the one nothing pinned: a revoke that overshot inside the target — the qualified
            // role, the constraints, the row policies — would satisfy every assertion above while
            // taking down the collector this database was migrated for. Asserted through the
            // repository, so the rollups' materialized views run too.
            writerRepositoryOn("mig", "mgt", "wmgt").persist(List.of(flow("mgt", "mgt-eu", 31101)));
            try (var reader = rawClientOn("mig", ProvisioningDdl.readerUser("mgt", "mig"), "rmgt")) {
                Assertions.assertThat(reader.queryAll("SELECT count() AS c FROM "
                                + FlowsSchema.qualifiedFlows("mig") + " WHERE srcPort = 31101")
                        .getFirst().getLong("c"))
                        .as("the migrated tenant's own reader must still read its rows")
                        .isEqualTo(1);
            }
    ...
    /**
     * A database with <em>no</em> row policies is refused, not read as "nobody is served".
     *
     * <p>The fixture is the {@code unmig} shape from
     * {@link #revokeLegacyClosesAMigratedDatabaseAndLeavesAnUnmigratedOneAlone}, and that is the
     * point: a hand-provisioned database with the legacy role granted on it, a live collector
     * authenticating as a pre-rename account, and no policy anywhere. The grantee check finds no
     * unqualified name there because it finds no name at all, so an empty result read as "nobody
     * depends on this" would take that collector's ingest away — the one outcome this command exists
     * to prevent, on a database it was never run against deliberately.</p>
     *
     * <p>Asserted with the write, not just the exit code: a refusal that revoked anyway is
     * indistinguishable from a correct one by exit code alone.</p>
     */
    @Test
    void revokeLegacyRefusesOnADatabaseWithNoRowPoliciesToReasonFrom() throws Exception {
        try (var admin = adminClient()) {
            admin.execute(FlowsSchema.createDatabase("nopol")).get();
            admin.execute(FlowsSchema.createFlowsTable("nopol")).get();
            admin.execute("CREATE ROLE IF NOT EXISTS flow_writer").get();
            admin.execute("GRANT INSERT, SELECT ON " + FlowsSchema.qualifiedFlows("nopol")
                    + " TO flow_writer").get();
            admin.execute("CREATE USER writer_nopol IDENTIFIED WITH sha256_password BY 'legacyW'").get();
            admin.execute("GRANT flow_writer TO writer_nopol").get();
            Assertions.assertThat(admin.queryAll("SELECT count() AS c FROM system.row_policies"
                            + " WHERE database = 'nopol'").getFirst().getLong("c"))
                    .as("the fixture is a database with no policies at all")
                    .isZero();

            final var err = new ByteArrayOutputStream();
            Assertions.assertThat(ProvisioningCommand.run(
                    revokeLegacyArgs("nopol"), discard(),
                    new PrintStream(err, true, StandardCharsets.UTF_8)))
                    .as("zero policies is 'I cannot tell', not 'nobody is served'")
                    .isEqualTo(1);
            Assertions.assertThat(err.toString(StandardCharsets.UTF_8))
                    .contains("NO row policies")
                    .contains("Nothing has been revoked");

            try (var legacy = rawClientOn("nopol", "writer_nopol", "legacyW")) {
                legacy.execute("INSERT INTO " + FlowsSchema.qualifiedFlows("nopol")
                        + " (tenant, organisation, timestamp) VALUES ('n', 'n-eu', now())").get();
            }
        }
    }

    /**
     * A policy applying to ALL is refused: it names nobody, so the grantee check cannot enumerate who
     * is served.
     *
     * <p>Measured on the pinned image: {@code CREATE ROW POLICY … TO ALL} stores
     * {@code apply_to_all = 1} with an <em>empty</em> {@code apply_to_list}, so a check reading only
     * the names concludes nobody is served on a database where everyone is — including the pre-rename
     * account this fixture keeps alive.</p>
     */
    @Test
    void revokeLegacyRefusesWhenAPolicyAppliesToEveryone() throws Exception {
        final String flows = FlowsSchema.qualifiedFlows("toall");
        try (var admin = adminClient()) {
            Assertions.assertThat(ProvisioningCommand.run(onboardArgsFor("toall", "tal", true),
                    discard(), discard())).isZero();
            admin.execute("CREATE ROLE IF NOT EXISTS flow_writer").get();
            for (final String table : ProvisioningDdl.legacyGrantTables("toall")) {
                admin.execute("GRANT INSERT, SELECT ON " + table + " TO flow_writer").get();
            }
            admin.execute("CREATE USER writer_toall IDENTIFIED WITH sha256_password BY 'legacyW'"
                    + " SETTINGS SQL_tenant = 'tal' CONST, SQL_org = 'tal-eu' CONST").get();
            admin.execute("GRANT flow_writer TO writer_toall").get();
            // Replaces the per-tenant policy with one naming nobody explicitly.
            admin.execute("CREATE ROW POLICY OR REPLACE tal_iso ON " + flows
                    + " FOR SELECT USING tenant = 'tal' TO ALL").get();
            Assertions.assertThat(admin.queryAll("SELECT length(apply_to_list) AS n FROM"
                            + " system.row_policies WHERE database = 'toall' AND table = 'flows'"
                            + " AND short_name = 'tal_iso'").getFirst().getLong("n"))
                    .as("TO ALL leaves the grantee list empty — the trap this refusal covers")
                    .isZero();

            final var err = new ByteArrayOutputStream();
            Assertions.assertThat(ProvisioningCommand.run(
                    revokeLegacyArgs("toall"), discard(),
                    new PrintStream(err, true, StandardCharsets.UTF_8))).isEqualTo(1);
            Assertions.assertThat(err.toString(StandardCharsets.UTF_8))
                    .contains("applies to ALL")
                    .contains("Nothing has been revoked");

            try (var legacy = rawClientOn("toall", "writer_toall", "legacyW")) {
                legacy.execute("INSERT INTO " + flows
                        + " (tenant, organisation, timestamp) VALUES ('tal', 'tal-eu', now())").get();
            }
        }
    }

    /**
     * A grant wider than these five tables is refused rather than half-revoked.
     *
     * <p>A per-table {@code REVOKE} does carve this database's tables out of a {@code ON <db>.*}
     * grant (measured), which is exactly why the probe must not simply ignore such a grant. But
     * running would leave the role holding everything else in the database while the success line
     * claims it "can no longer reach" it — and the wider row survives with
     * {@code is_partial_revoke = 0}, so no re-run would ever be the promised no-op. Both are asserted:
     * the refusal, and that a re-run of the same command says the same thing rather than drifting.</p>
     */
    @Test
    void revokeLegacyRefusesAGrantWiderThanTheTablesItRevokes() throws Exception {
        try (var admin = adminClient()) {
            Assertions.assertThat(ProvisioningCommand.run(onboardArgsFor("wide", "wid", true),
                    discard(), discard())).isZero();
            admin.execute("CREATE ROLE IF NOT EXISTS flow_writer").get();
            admin.execute("GRANT INSERT, SELECT ON wide.* TO flow_writer").get();

            final var err = new ByteArrayOutputStream();
            Assertions.assertThat(ProvisioningCommand.run(
                    revokeLegacyArgs("wide"), discard(),
                    new PrintStream(err, true, StandardCharsets.UTF_8))).isEqualTo(1);
            final String refused = err.toString(StandardCharsets.UTF_8);
            Assertions.assertThat(refused)
                    .contains("WIDER")
            ...
            // Nothing was revoked, so the wider grant is still whole and a re-run is identical.
            Assertions.assertThat(admin.queryAll("SELECT count() AS c FROM system.grants"
                            + " WHERE role_name = 'flow_writer' AND database = 'wide'"
                            + " AND table IS NULL AND is_partial_revoke = 0").getFirst().getLong("c"))
                    .isPositive();
            final var again = new ByteArrayOutputStream();
            Assertions.assertThat(ProvisioningCommand.run(
                    revokeLegacyArgs("wide"), discard(),
                    new PrintStream(again, true, StandardCharsets.UTF_8))).isEqualTo(1);
            Assertions.assertThat(again.toString(StandardCharsets.UTF_8)).contains("WIDER");
        }
    }

    /**
     * A typo'd {@code --database} must not exit 0 with an all-clear.
     *
     * <p>Every check downstream reads a nonexistent database as clean — no grants match, no policies
     * match — so without a pre-flight the operator is told "nothing to revoke", offered two
     * explanations neither of which is the true one, and leaves the real database untouched. That is
     * the worst available failure for a command whose only job is closing an exposure, and
     * {@code onboard} guards exactly this (#267).</p>
     */
    @Test
    void revokeLegacyOnAMistypedDatabaseRefusesInsteadOfReportingAllClear() {
        final var err = new ByteArrayOutputStream();
        Assertions.assertThat(ProvisioningCommand.run(
                revokeLegacyArgs("ript1de"), discard(),
                new PrintStream(err, true, StandardCharsets.UTF_8)))
                .as("a database that does not exist must not report an all-clear")
                .isEqualTo(1);
        final String refused = err.toString(StandardCharsets.UTF_8);
        Assertions.assertThat(refused)
                .contains("ript1de")
                .contains("typos");
        Assertions.assertThat(refused)
                .as("and must not be phrased as the successful no-op it is not")
                .doesNotContain("Nothing to revoke");
    }
    ...
    private static String[] revokeLegacyArgs(final String database) {
        return new String[] {"revoke-legacy", "--admin-url", endpoint(), "--database", database, "--yes"};
    }

    private static String[] revokeLegacyDryRunArgs(final String database) {
    ...
    private static String[] revokeLegacyArgs(final String database, final String adminUser,
        ...
        return new String[] {"revoke-legacy", "--admin-url", endpoint(), "--database", database,
                "--admin-user", adminUser, "--admin-password", adminPassword, "--yes"};
    }
    ...
    private static ClickhouseRepository writerRepository(final String tenant, final String password) {
        return writerRepositoryOn(DATABASE, tenant, password);
    }

    private static ClickhouseRepository writerRepositoryOn(final String database, final String tenant,
