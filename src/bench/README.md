...
Asserted today (reference-implementation properties; a failure means the harness or environment broke, not that production regressed):

| Assertion | Meaning | Threshold | Baseline (2026-08-13, M-series laptop) |
|---|---|---|---|
| `lookup.trie-scale-flatness` | Reference trie ns/op at 10k entries vs at 100 entries | ≤ 6.0 | 2.1 |
| `parse.direct-linearity` | Direct-parse per-entry cost at 100k entries vs at 10k | ≤ 3.0 | 0.7 |

Deliberately **not** asserted at this stage:

- Registry-vs-trie speedup: breaks by design when trie-backed matching lands in production (story 1.4).
...
- **Story 1.4** (trie matching) activates the production-lookup budget: production lookup ns/op within a small factor of the reference trie ns/op, same run.
...
3. Set the threshold at roughly 3x the baseline and record both in the constant's javadoc (`TRIE_FLATNESS_MAX`, `DIRECT_LINEARITY_MAX`) and in the table above.
