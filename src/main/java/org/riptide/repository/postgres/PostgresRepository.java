...
public class PostgresRepository implements FlowRepository {
    ...
    private void persistBuckets(List<EnrichedFlow> list) {
        ...
        new JdbcTemplate(dataSource).batchUpdate(PostgresQueries.QUERY_INSERT_BUCKETS,
                new PostgresBucketPreparedStatementSetter(buckets));
    }

    private void persistFlows(final List<EnrichedFlow> flows) {
        new JdbcTemplate(dataSource).batchUpdate(PostgresQueries.QUERY_INSERT_FLOWS,
                new PostgresFlowPreparedStatementSetter(() -> flows));
    }
}
