...
echo 'CLICKHOUSE_PASSWORD=change-me-before-anyone-else-can-reach-this-host' > .env
docker compose up -d
```
...
* ClickHouse on `127.0.0.1:8123` as user `default` with the `CLICKHOUSE_PASSWORD` from `.env`. It is required, and the whole stack follows it. The file is gitignored, and every `docker compose` command in that directory reads it.
